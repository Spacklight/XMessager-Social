 ...
 *   env.HF_TOKEN       - Hugging Face access token, secret
 *   env.HF_MEDIA_REPO  - e.g. "Spacklight/Social-Media"
 */

const SESSION_TTL_MS = 30 * 24 * 60 * 60 * 1000;
const MAX_MEDIA_BYTES = 20 * 1024 * 1024; // 20MB for chat/group/page attachments
...
// ---------- media upload (used by groups/pages/chats) ----------
...
async function handleMediaUpload(request, env, cors) {
  ...
  let oid;
  try {
    ({ oid } = await uploadViaLfs(env, env.HF_MEDIA_REPO, buf, path));
  } catch (err) {
    return json({ error: "Hugging Face LFS upload failed", details: err.message }, 502, cors);
  }

  const commitBody =
    JSON.stringify({ key: "header", value: { summary: `Upload media ${id}`, description: `uploader=${user.id}` } }) +
...
async function getGroupMembership(db, groupId, userId) {
  return await db.prepare(`SELECT * FROM group_members WHERE group_id=? AND user_id=?`).bind(groupId, userId).first();
}
...
async function isFollower(db, pageId, userId) {
  return await db.prepare(`SELECT * FROM page_followers WHERE page_id=? AND user_id=?`).bind(pageId, userId).first();
}
...
async function listChats(request, env, cors) {
  const user = await requireAuth(request, env);
  if (!user) return json({ error: "Unauthorized" }, 401, cors);
  const { results } = await env.DB.prepare(`SELECT * FROM chats WHERE user_a=? OR user_b=? ORDER BY created_at DESC LIMIT 50`).bind(user.id, user.id).all();
  return json({ chats: results }, 200, cors);
}
